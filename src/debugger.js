...
* @Last Modified time: 2018-10-10 16:48:41
*/

import { Selector } from './debugger-selector.js';

import { Styles } from './debugger-css.js';
import { ExtendedStyles } from './debugger-css-extended.js';
...
outterContainer.innerHTML += '<div id="debuggerPanel-resize" style="'+ debuggerResizerCss +'"></div>';
outterContainer.innerHTML += '<style>'+debuggerDocumentStyles+'</style>';
outterContainer.innerHTML += '<div id="debuggerPanel-viewport"></div>';
document.body.appendChild(outterContainer);

const innerContainer         = document.querySelector('#debuggerPanel-viewport').attachShadow({mode: 'open'});
innerContainer.innerHTML     = debugerUI;

const resizeBar              = document.querySelector('#debuggerPanel-resize');
